...
class TransferAPI {
    ...
    public static function transferPlayer(Player $player, string $servername){
        ...
        $pk->port = 0; //The server port you specified in the WaterDogPE config.
        $player->sendDataPacket($pk);
        Server::getInstance()->getLogger()->info($player->getName() . "§c will teleported to Server §e{$servername}§8.");
    }
